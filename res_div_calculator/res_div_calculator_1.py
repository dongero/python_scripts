"""TO DO
-import resistor list from kicad component list?
-remember how to make classes/defs to clean up metric
-how to make lists/append lists
"""

rat_temp = 0
rat_upper = 100000
r1_upper = 0
r2_upper = 0
r1_upper_list = []
r2_upper_list = []
rat_lower = 0
r1_lower = 0
r2_lower = 0
r1_lower_list = []
r2_lower_list = []

count1 = 0
count2 = 0

#import from external list - make option to include common or uncommon values
resistors =  [
10.0, 10.2, 10.5, 10.7, 11.0, 11.3, 11.5, 11.8, 12.1, 12.4, 12.7, 13.0, 13.3, 13.7, 14.0, 14.3, 14.7, 15.0, 15.4, 15.8, 16.2, 16.5, 16.9, 17.4, 17.8, 18.2, 18.7, 19.1, 19.6, 20.0, 20.5, 21.0, 21.5, 22.1, 22.6, 23.2, 23.7, 24.3, 24.9, 25.5, 26.1, 26.7, 27.4, 28.0, 28.7, 29.4, 30.1, 30.9, 31.6, 32.4, 33.2, 34.0, 34.8, 35.7, 36.5, 37.4, 38.3, 39.2, 40.2, 41.2, 42.2, 43.2, 44.2, 45.3, 46.4, 47.5, 48.7, 49.9, 51.1, 52.3, 53.6, 54.9, 56.2, 57.6, 59.0, 60.4, 61.9, 63.4, 64.9, 66.5, 68.1, 69.8, 71.5, 73.2, 75.0, 76.8, 78.7, 80.6, 82.5, 84.5, 86.6, 88.7, 90.9, 93.1, 95.3, 97.6, 100, 102, 105, 107, 110, 113, 115, 118, 121, 124, 127, 130, 133, 137, 140, 143, 147, 150, 154, 158, 162, 165, 169, 174, 178, 182, 187, 191, 196, 200, 205, 210, 215, 221, 226, 232, 237, 243, 249, 255, 261, 267, 274, 280, 287, 294, 301, 309, 316, 324, 332, 340, 348, 357, 365, 374, 383, 392, 402, 412, 422, 432, 442, 453, 464, 475, 487, 499, 511, 523, 536, 549, 562, 576, 590, 604, 619, 634, 649, 665, 681, 698, 715, 732, 750, 768, 787, 806, 825, 845, 866, 887, 909, 931, 953, 976, 1000, 1020, 1050, 1070, 1100, 1130, 1150, 1180, 1210, 1240, 1270, 1300, 1330, 1370, 1400, 1430, 1470, 1500, 1540, 1580, 1620, 1650, 1690, 1740, 1780, 1820, 1870, 1910, 1960, 2000, 2050, 2100, 2150, 2210, 2260, 2320, 2370, 2430, 2490, 2550, 2610, 2670, 2740, 2800, 2870, 2940, 3010, 3090, 3160, 3240, 3320, 3400, 3480, 3570, 3650, 3740, 3830, 3920, 4019, 4120, 4220, 4320, 4420, 4530, 4640, 4750, 4870, 4990, 5110, 5230, 5360, 5490, 5620, 5760, 5900, 6040, 6190, 6340, 6490, 6650, 6810, 6980, 7150, 7320, 7500, 7680, 7870, 8060, 8250, 8450, 8660, 8870, 9090, 9310, 9530, 9760, 10000, 10200, 10500, 10700, 11000, 11300, 11500, 11800, 12100, 12400, 12700, 13000, 13300, 13700, 14000, 14300, 14700, 15000, 15400, 15800, 16200, 16500, 16900, 17400, 17800, 18200, 18700, 19100, 19600, 20000, 20500, 21000, 21500, 22100, 22600, 23200, 23700, 24300, 24900, 25500, 26100, 26700, 27400, 28000, 28700, 29400, 30100, 30900, 31600, 32400, 33200, 34000, 34800, 35700, 36500, 37400, 38300, 39200, 40200, 41200, 42200, 43200, 44200, 45300, 46400, 47500, 48700, 49900, 51100, 52300, 53600, 54900, 56200, 57600, 59000, 60400, 61900, 63400, 64900, 66500, 68100, 69800, 71500, 73200, 75000, 76800, 78700, 80600, 82500, 84500, 86600, 88700, 90900, 93100, 95300, 97600, 100000, 102000, 105000, 107000, 110000, 113000, 115000, 118000, 121000, 124000, 127000, 130000, 133000, 137000, 140000, 143000, 147000, 150000, 154000, 158000, 162000, 165000, 169000, 174000, 178000, 182000, 187000, 191000, 196000, 200000, 205000, 210000, 215000, 221000, 226000, 232000, 237000, 243000, 249000, 255000, 261000, 267000, 274000, 280000, 287000, 294000, 301000, 309000, 316000, 324000, 332000, 340000, 348000, 357000, 365000, 374000, 383000, 392000, 402000, 412000, 422000, 432000, 442000, 453000, 464000, 475000, 487000, 499000, 511000, 523000, 536000, 549000, 562000, 576000, 590000, 604000, 619000, 634000, 649000, 665000, 681000, 698000, 715000, 732000, 750000, 768000, 787000, 806000, 825000, 845000, 866000, 887000, 909000, 931000, 953000, 976000, 1000000, 1020000, 1050000, 1070000, 1100000, 1130000, 1150000, 1180000, 1210000, 1240000, 1270000, 1300000, 1400000, 2000000]

print("-----------------------")
vin = float(input("Vin: "))
vout = float(input("Vout: "))
ratio = vout / vin
print("Ratio: ", round(ratio,2))


for x in resistors:
    for y in resistors:
        rat_temp = y / (x + y)
        if rat_temp >= ratio and rat_temp <= rat_upper:
            rat_upper = rat_temp
            vol_upper = rat_temp * vin
            r1_upper = str(x)
            r2_upper = str(y)

# need to figure out how to set this up as a class or def?
            if x < 1000:
                r1_upper = r1_upper + "R" 
            elif x < 1000000:
                r1_upper = str(x/1000) + "K"
            else:
                r1_upper = str(x/1000000) + "M"

            if y < 1000: 
                r2_upper = r2_upper + "R" 
            elif y < 1000000:
                r2_upper = str(y/1000) + "K"
            else:
                r2_upper = str(y/1000000) + "M"

        if rat_temp <= ratio and rat_temp >= rat_lower:
            rat_lower = rat_temp
            vol_lower = rat_temp * vin

            if x < 1000:
                r1_lower = str(r1_lower) + "R" 
            elif x < 1000000:
                r1_lower = str(x/1000) + "K"
            else:
                r1_lower = str(x/1000000) + "M"

            if y < 1000:
                r2_lower = str(r2_lower) + "R" 
            elif y < 1000000:
                r2_lower = str(y/1000) + "K"
            else:
                r2_lower = str(y/1000000) + "M"

print("-----------------------\nUpper Voltage:",round(vol_upper,2), "V  \nRatio =", round(rat_upper,2), "\nR1 =", r1_upper, "\nR2 =", r2_upper)
print("-----------------------\nLower Voltage:",round(vol_lower,2), "V  \nRatio =", round(rat_lower,2), "\nR1 =", r1_lower, "\nR2 =", r2_lower)
        
        




